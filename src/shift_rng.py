...
def lfsr(seed, count):
    ...
    for _ in range(100000):
        register >>= 1
        register = update_register(register, length, [position_0, position_1, position_2, position_3, position_4, position_5, position_6])
    
    # Build random list
    for _ in range(count):
        lfsr_rn.append(register & 1)
        register >>= 1
        register = update_register(register, length, [position_1, position_2, position_3, position_4, position_5])
...
def main():
    # seed = get_seed()
    seed = 98403759082439875973490510938409
    count = 10000
